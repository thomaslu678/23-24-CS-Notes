...
public class TaxReturn {
    ...
    public double getTax() {
        ...
        if (maritalStatus == SINGLE){
            if (this.income <= RATE1_SINGLE_LIMIT){
                tax1 = RATE1 * this.income;
            }
            else{
                tax1 = RATE1 * RATE1_SINGLE_LIMIT;
                tax2 = RATE2 * (this.income = RATE1_SINGLE_LIMIT);
            }
        }
        else{
            if (this.income <= RATE1_MARRIED_LIMIT){
                tax1 = RATE1 * this.income;
            }
            else{
                tax1 = RATE1 * RATE1_MARRIED_LIMIT;
                tax2 = RATE2 * (this.income = RATE1_MARRIED_LIMIT);
            }
        }
